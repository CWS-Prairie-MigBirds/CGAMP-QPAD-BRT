...
AOU <- read.csv("Data/BRT_Input/AOU_SpCodes.csv", stringsAsFactors = F)
IBP <- read.csv("Data/BRT_Input/IBPSpeciesCodes.csv", stringsAsFactors = F)
...
load(paste0("Data/BRT_Input/wDrought/",species[1],".RData"))
...
done <- list.files("Output/Models_bootstrap")
...
gbm.boot <- function(x) {
  ...
  load(paste0("Data/BRT_Input/wDrought/",x$species,".RData"))
  ...
  saveRDS(model,file=paste0("Output/Models_bootstrap/",x$species, "_boot", x$bootsample, ".RData"))
}
...
clusterEvalQ(cl, sink(paste0("Output/", Sys.getpid(), ".txt"))) #sink all text from console to txt files
