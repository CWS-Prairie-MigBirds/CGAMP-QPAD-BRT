...
file.list <- list.files("Data/SpatialCovariates", pattern = "tif|nc", recursive = T, full.names = T)

#import file.list into raster.list
raster.list <- rast(file.list)
...
for (i in years) {
  ...
  input.list[[i]] <- list(stack=stack.list[[i]],pts=xyTemp,pkey=pkeyTemp) for
