...
for (i in 1:length(model.list)) {
  ...
  if(file.exists(paste0("Output/Models_hyperparams/",file))) {
    print(paste0(file, " already exists. Removing model from list and checking for results in CV stats table..."))
    remove <- append(remove, i)
    if(file.exists("Output/CV_Stats.csv")) {
      cvstats = read.csv("Output/CV_Stats.csv")
      if(nrow(cvstats[((cvstats$Species==model.list[[i]]$sp)&(cvstats$ModelName==file)),])>0) {
        ...
        load(paste0("Output/Models_hyperparams/",file))
        test = "GO"
        }
    } else {
  ...
  if(test=="GO") {
    ...
    if(file.exists("Output/CV_Stats.csv")) {
      write.table(cvstats, file="Output/CV_Stats.csv", sep = ",", append=T,
                  quote=F,col.names=F,row.names=F)
    } else {write.csv(cvstats,file="Output/CV_Stats.csv",row.names=F)}
  }
}
...
BRT.par <- function(x) {
  load(paste0("Data/BRT_Input/wDrought/", x$sp, ".RData"))
  ...
  if(file.exists("Output/CV_Stats.csv")) {
    write.table(cvstats, file="Output/CV_Stats.csv", sep = ",", append=T,
                quote=F,col.names=F,row.names=F)
  } else {write.csv(cvstats,file="AOutput/CV_Stats.csv",row.names=F)}
  
  print("Saving model...")
  save(model,file=paste0("Output/Models_hyperparams/",file))
}
...
cvstats <- read.csv("Output/CV_Stats.csv",stringsAsFactors = F)
...
saveRDS(top.models, file = "Output/TopModels.rds")
...
cvstats <- read.csv("Output/CV_Stats.csv",stringsAsFactors = F)
top.models <- readRDS("Output/TopModels.rds")
...
write.csv(cvTop, "Output/cvStatsTopModels.csv", row.names = F)
